...
obscenity_words = set(json.load((dataset_dir / "obscenity_words.json").open(), encoding="utf-8"))
obscenity_words_extended = set(json.load((dataset_dir / "obscenity_words_extended.json").open(), encoding="utf-8"))
obscenity_words_exception = set(json.load((dataset_dir / "obscenity_words_exception.json").open(), encoding="utf-8"))
obscenity_words.update(obscenity_words_extended)
word_pattern = re.compile(r"[А-яЁё]+")


def insult_classifier(text):
    for word in word_pattern.findall(text):
        ...
        if normal_word in obscenity_words_exception or word in obscenity_words_exception:
            continue
        if (
            normal_word in obscenity_words
